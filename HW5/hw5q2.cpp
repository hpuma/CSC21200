...
// Make sure there is at least ONE NODE that exists in the tree.
template<class Item>
btNode<Item>* binaryTree<Item>::getNode(){
    if(size() > 0){
        return curr; // Returns the node that the curr pointer is pointing at.
    }
}
...
// Gets the data where the current node is pointing to.
template<class Item>
...
// Return a boolean signifying whether or not a LEFT child exists.
template<class Item>
...
// Return a boolean signifying whether or not a RIGHT child exists.
template<class Item> 
